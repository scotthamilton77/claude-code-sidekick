...
### 3.5 Monitoring UI Integration

The Session Summary feature emits **Entity-Lifecycle events** (see `LLD-STRUCTURED-LOGGING.md`) for the `summary` entity. Events are logged to `.sidekick/sessions/{session_id}/events.jsonl`:
...
// Analysis triggered (user prompt or countdown reached zero)
{
  ...
  "lifecycle": "analyzing",
  "reason": "user_prompt",
  "metadata": { "countdown": 0, "confidence": 0.5 }
}
...
// Analysis skipped (countdown still active)
{
  ...
  "metadata": { "countdown": 5, "skipped": true }
}

// Summary updated (generation complete)
{
  "source": "sidekick-supervisor",
  "pid": 12345,
  "context": {
  ...
  "state": {
    ...
    "new_intent": "Fixing the token expiration bug"
  }
}
```

The `state` field contains the complete `session-summary.json` content (stored at `.sidekick/sessions/{session_id}/state/session-summary.json`), enabling full reconstruction in the Monitoring UI.
...
1.  **Snarky Commentary**: The legacy script had hooks for "snarky comments". Is this a separate feature (`feature-personality`) or part of this one?
    - _Recommendation_: Keep it separate. `feature-session-summary` produces the _data_. `feature-personality` (if we build it) _consumes_ that data to generate commentary.
2.  **Sleeper**: The legacy script launched a "sleeper" process.
    - _Recommendation_: The Supervisor replaces the ad-hoc "sleeper". The Supervisor can schedule delayed tasks (e.g., "If no activity for 5 mins, generate a 'bored' comment").
