 ...
 * via the OpenAI API. It supports:
 * - Configurable timeout with AbortController
 * - Retry logic for transient failures (timeout, rate limit)
 * - Structured output with JSON schema enforcement
 * - Full metadata extraction (tokens, timing)
 * - JSON extraction from responses
 */

import OpenAI from 'openai'
import { LLMProvider } from './LLMProvider.js'
import { OpenAIConfig, InvokeOptions, LLMResponse, LLMErrorType, TokenUsage } from './types.js'
 ...
 * ```
 */
export class OpenAIProvider extends LLMProvider<OpenAIConfig> {
  ...
  constructor(config: OpenAIConfig) {
    ...
    if (!config.apiKey) {
      throw this.createError(
        ...
        'OpenAI API key not found. Set apiKey in config or OPENAI_API_KEY environment variable'
      )
    }

    // Extract base URL from endpoint if provided
        ...
        try {
          ...
          if (choice.message.content) {
            ...
            if (toolCall && 'function' in toolCall) {
              if (toolCall.function.name === '<|constrain|>json') {
                ...
                throw this.createError(
                  LLMErrorType.API_ERROR,
                  `OpenAI API returned unexpected tool call: ${toolCall.function.name}`,
                  undefined,
                  {
              ...
              throw this.createError(
                LLMErrorType.API_ERROR,
                'OpenAI API returned non-function tool call',
                undefined,
                {
                  toolCalls: choice.message.tool_calls,
                }
            ...
            throw this.createError(
              LLMErrorType.API_ERROR,
              'OpenAI API returned empty content and no tool_calls',
              undefined,
              {
                choice,
              }
          ...
          const response: LLMResponse = {
            ...
            metadata: {
              ...
              // Note: OpenAI doesn't provide cost in API response
