...
**IMPORTANT:** This command integrates with the task management system in `/planning/tasks/` and coordinates with multi-agent workflows.
...
1. **Parse Arguments and Validate Task Path**:
   ...
   const taskFile = `/planning/tasks/${projectName}/${getTaskIndex(
     taskName
   )}-${taskName}.md`;
   ```
...
2. **Load Task and Project Status**:
   ...
   const statusFile = `/planning/tasks/${projectName}/status.json`;
