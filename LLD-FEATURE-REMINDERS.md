...
### 6.1 Monitoring UI Integration

The `ReminderService` emits **Entity-Lifecycle events** (see `LLD-STRUCTURED-LOGGING.md`) for each reminder instance. Events are logged to `.sidekick/sessions/{session_id}/events.jsonl`:

```json
// Reminder created (at session start or when dynamically added)
{
...
// Reminder triggered (countdown reached zero)
{
  ...
  "state": { "countdown": 0, "template": "user-prompt-submit", "text": "..." }
}

// Reminder injected into hook response
{
  "source": "sidekick-supervisor",
  "pid": 12345,
  "context": {
...
// Reminder dismissed (e.g., stuck reminder cleared after stop allowed)
{
  "source": "sidekick-supervisor",
  "pid": 12345,
  "context": {
