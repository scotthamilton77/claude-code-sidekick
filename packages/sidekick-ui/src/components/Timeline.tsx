    ...
    return (
        <div className="w-28 bg-white border-r border-slate-200 flex flex-col">
                    ...
                    {events.map((event) => {
                        ...
                        return (
                            <button
                                ...
                                disabled={isFiltered}
                                className={`relative flex items-center transition-all ${isFiltered ? 'opacity-20' : ''
                                    } ${isFuture ? 'opacity-40' : ''}`}
                                title={`${event.time} - ${event.label}`}
                            >
                                {/* Small colored dot */}
                                <div
                                    className={`w-3 h-3 rounded-full -ml-1 transition-all ${isActive
                                ...
                                <span className={`ml-3 text-xs font-mono whitespace-nowrap ${isActive ? 'text-indigo-600 font-medium' : 'text-slate-400'
                                    }`}>
                                    {event.time.slice(-5)}
                                </span>
                            </button>
            ...
            <div className="p-2 border-t border-slate-100 text-center">
                <span className="text-xs text-slate-500">{currentEventId + 1} / {events.length}</span>
            </div>
        </div>
    );
};
