...
export function registerStageDefaultUserPrompt(context: RuntimeContext): void {
  ...
  context.handlers.register({
    ...
    handler: async (event) => {
      ...
      const sessionId = event.context.sessionId
      if (!sessionId) return
      await resetThrottleCounters(daemonCtx, sessionId)
    },
  })
  ...
  context.handlers.register({
    ...
    handler: async (event) => {
      ...
      const featureConfig = daemonCtx.config.getFeature<RemindersSettings>('reminders')
      ...
      for (const [reminderId, entry] of Object.entries(state)) {
        ...
        if (newCount >= threshold) {
          ...
          await stageReminder(daemonCtx, typedEntry.targetHook, {
