...
- Writes `resume-message.json` in current session directory
...
2. Finds most recent session with `resume-message.json` and `clarity_score >= RESUME_MIN_CLARITY`
3. Reads `resume-message.json` fields (generated by previous session's session summary)
...
**Input**: Previous session's `resume-message.json` (created by `resume_generate_async()` in topic-extraction.sh)

**resume-message.json Schema**:
...
4. If `session_title` or `latest_intent` changed AND both confidence scores >= `SLEEPER_MIN_CONFIDENCE`:
   - Invokes `generate-resume.sh` to create/update resume-message.json
...
**Output**: Creates/updates `${session_dir}/resume-message.json` via generate-resume.sh when conditions met
...
cleanup_launch() → process_launch_background → nohup cleanup_run &
  ↓
resume_snarkify() → reads previous resume-message.json → writes ${session_dir}/session-summary.json
  ↓
...
if either countdown <= 0:
  ...
  save updated state
  ↓
  writes ${session_dir}/resume-message.json
  ↓
  [NEXT session's SessionStart will use this resume-message.json]
```
...
- Natural first-session handling (no resume-message.json = graceful skip)
