...
1.  **Read Shared State**: Consume `session-state.json` and `session-summary.json` generated by the Daemon.
...
- **Daemon** watches the transcript and updates `session-state.json` with the current token count and cost.
