...
The user does not see the generation process. It happens asynchronously in the Daemon as they work. When their session topic stabilizes (significant title change with high confidence), the system quietly prepares the resume artifacts for the _next_ time they return.

## 3. Architecture

The feature is split between **statusline.sh** (consumption) and the **Daemon** (production).
...
### 3.2 Daemon: Artifact Generation (Producer)

The Daemon generates resume artifacts via the `UpdateSessionSummary` handler.
...
Generated by the Daemon, consumed by statusline.sh in the _next_ session.
...
The Resume feature emits `SidekickEvent` records (see `docs/design/flow.md` §3.2) to the Daemon log (`sidekickd.log`, per `docs/design/STRUCTURED-LOGGING.md`):
